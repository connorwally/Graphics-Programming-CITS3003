...
void PanningCamera::update(const Window& window, float dt, bool controls_enabled) {
    ...
    // Compute rotation matrix based on pitch and yaw angles
    glm::mat4 rotation_matrix = glm::rotate(glm::mat4{1.0f}, pitch, glm::vec3{1.0f, 0.0f, 0.0f});
    rotation_matrix = glm::rotate(rotation_matrix, yaw, glm::vec3{0.0f, 1.0f, 0.0f});

    // Compute rotated camera position vector (initially the negative of the z-axis)
    glm::vec4 rotated_camera_pos = glm::vec4{0.0f, 0.0f, -distance, 1.0f} * rotation_matrix;

    // Translate rotated camera position vector to focus point
    glm::vec3 camera_pos = glm::vec3{rotated_camera_pos.x, rotated_camera_pos.y, rotated_camera_pos.z} + focus_point;
