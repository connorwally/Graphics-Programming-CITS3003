...
void point_light_calculation(PointLightData point_light, LightCalculatioData calculation_data, float shininess, inout vec3 total_diffuse, inout vec3 total_specular, inout vec3 total_ambient) {
    ...
    float distance = length(ws_light_offset); // computes distance between light and fragment

    // the following calculation is derived from messing around with different formulas and seeing which looks the best
    //float attenuation = 1.0 / (distance * (distance / 2));
    // the following calculation and values of 0.09 and 0.032 were provided by chatGPT
    //float attenuation = 1.0 / (1.0 + 0.09 * distance + 0.032 * distance * distance);
    // the following calculation was retrieved from section 7.2.7 of "Introduction to Computer Graphics, v1.3" by David J. Eck
